...
"#;
    ...
    // Let's test without ORDER BY first
    ...
    match stmt {
        vibesql_ast::Statement::Select(select_stmt) => {
            ...
            println!("Without ORDER BY (first 5):");
    ...
    // Test with ORDER BY without CASE
    let query_sql = "SELECT a*2 FROM t1 ORDER BY 1";
    let stmt = Parser::parse_sql(query_sql).unwrap();
    match stmt {
        vibesql_ast::Statement::Select(select_stmt) => {
            let executor = SelectExecutor::new(&db);
