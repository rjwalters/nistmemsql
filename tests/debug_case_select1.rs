...
"#;
    ...
    match stmt {
        vibesql_ast::Statement::Select(select_stmt) => {
            ...
            for row in &rows {
                match &row.values[0] {
                    vibesql_types::SqlValue::Integer(n) => println!("{}", n),
                    vibesql_types::SqlValue::Numeric(n) => println!("Numeric: {}", n),
                    _ => println!("Other: {:?}", row.values[0]),
                }
            }
            ...
            vals.sort();
            let is_sorted = rows.iter().zip(vals.iter()).all(|(row, expected)| {
                match &row.values[0] {
                    vibesql_types::SqlValue::Integer(n) => n == expected,
                    _ => false,
                }
            });
