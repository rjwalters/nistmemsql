...
fn test_count_star_in_multiplication() {
    ...
    // -18 * COUNT(*) = -18 * 2 = -36
    assert_eq!(result[0].values[0], types::SqlValue::Float(-36.0));
}
...
fn test_count_star_in_addition() {
    ...
    assert_eq!(result.len(), 1);
    // COUNT(*) + COUNT(*) = 3 + 3 = 6
    assert_eq!(result[0].values[0], types::SqlValue::Float(6.0));
}
...
fn test_count_star_complex_expression() {
    ...
    assert_eq!(result[0].values[0], types::SqlValue::Float(13.0));
}
...
fn test_count_star_with_unary_operators() {
    ...
    assert_eq!(result[0].values[0], types::SqlValue::Float(300.0));
}
...
fn test_count_star_with_negative_unary() {
    ...
    assert_eq!(result.len(), 1);
    // - COUNT(*) * 9 = -3 * 9 = -27
    assert_eq!(result[0].values[0], types::SqlValue::Float(-27.0));
}
