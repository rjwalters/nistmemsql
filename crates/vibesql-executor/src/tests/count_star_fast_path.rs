...
fn test_count_star_fast_path_empty_table() {
    ...
    let result = executor.execute(&stmt).unwrap();
    assert_eq!(result.len(), 1);
    assert_eq!(result[0].values[0], vibesql_types::SqlValue::Numeric(0.0));
}
...
fn test_count_star_with_where_no_fast_path() {
    ...
    let stmt = vibesql_ast::SelectStmt {
        ...
        where_clause: Some(vibesql_ast::Expression::BinaryOp {
            ...
            right: Box::new(vibesql_ast::Expression::Literal(vibesql_types::SqlValue::Numeric(5.0))),
        }),
        group_by: None,
        having: None,
        order_by: None,
    ...
    assert_eq!(result[0].values[0], vibesql_types::SqlValue::Numeric(7.0));
}
...
fn test_count_star_distinct_no_fast_path() {
    ...
    let result = executor.execute(&stmt).unwrap();
    assert_eq!(result.len(), 1);
    assert_eq!(result[0].values[0], vibesql_types::SqlValue::Numeric(5.0));
}
...
fn test_count_star_with_alias() {
    ...
    let result = executor.execute(&stmt).unwrap();
    assert_eq!(result.len(), 1);
    assert_eq!(result[0].values[0], vibesql_types::SqlValue::Numeric(100.0));
}
