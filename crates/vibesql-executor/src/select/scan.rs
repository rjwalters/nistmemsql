...
fn count_tables_in_from(from: &vibesql_ast::FromClause) -> usize {
    match from {
...
fn all_joins_are_inner_or_cross(from: &vibesql_ast::FromClause) -> bool {
    match from {
    ...
    F: Fn(&vibesql_ast::SelectStmt) -> Result<Vec<vibesql_storage::Row>, ExecutorError> + Copy,
{
    ...
    match from {
        ...
        vibesql_ast::FromClause::Join { left, right, join_type, condition } => execute_join(
            left,
            right,
            join_type,
            condition,
