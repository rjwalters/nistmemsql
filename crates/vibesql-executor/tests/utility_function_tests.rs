...
fn test_substr_to_end() {
    ...
    let result = evaluator.eval(&expr, &row).unwrap();
    assert_eq!(result, vibesql_types::SqlValue::Varchar("World".to_string()));
}
...
fn test_format_basic() {
    ...
    let result = evaluator.eval(&expr, &row).unwrap();
    assert_eq!(result, vibesql_types::SqlValue::Varchar("1,234,567.89".to_string()));
}
...
fn test_format_adds_zeros() {
    ...
    let result = evaluator.eval(&expr, &row).unwrap();
    assert_eq!(result, vibesql_types::SqlValue::Varchar("42.00".to_string()));
}
