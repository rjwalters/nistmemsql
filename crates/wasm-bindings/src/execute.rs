...
pub fn execute_statement(db: &mut Database, sql: &str) -> Result<JsValue, JsValue> {
    ...
    match stmt {
        ...
        ast::Statement::BeginTransaction(begin_stmt) => {
            let message = executor::BeginTransactionExecutor::execute(&begin_stmt, &mut db.db)
                .map_err(|e| JsValue::from_str(&format!("Execution error: {:?}", e)))?;
        ...
        ast::Statement::Savepoint(savepoint_stmt) => {
            let message = executor::SavepointExecutor::execute(&savepoint_stmt, &mut db.db)
                .map_err(|e| JsValue::from_str(&format!("Execution error: {:?}", e)))?;
        ...
        ast::Statement::ReleaseSavepoint(release_stmt) => {
            let message = executor::ReleaseSavepointExecutor::execute(&release_stmt, &mut db.db)
                .map_err(|e| JsValue::from_str(&format!("Execution error: {:?}", e)))?;
        ...
        ast::Statement::DropRole(drop_role_stmt) => {
            let message = executor::RoleExecutor::execute_drop_role(&drop_role_stmt, &mut db.db)
                .map_err(|e| JsValue::from_str(&format!("Execution error: {:?}", e)))?;
        ...
        ast::Statement::CreateDomain(create_domain_stmt) => {
            let message =
                executor::DomainExecutor::execute_create_domain(&create_domain_stmt, &mut db.db)
                    .map_err(|e| JsValue::from_str(&format!("Execution error: {:?}", e)))?;
        ...
        ast::Statement::DropType(drop_type_stmt) => {
            let message = executor::TypeExecutor::execute_drop_type(&drop_type_stmt, &mut db.db)
                .map_err(|e| JsValue::from_str(&format!("Execution error: {:?}", e)))?;
