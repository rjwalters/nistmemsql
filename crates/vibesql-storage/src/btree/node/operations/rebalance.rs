//! Rebalancing operations for BTreeIndex
//!
//! This module contains all logic for rebalancing the B+ tree after deletions,
//! including borrowing from siblings, merging nodes, and collapsing the root.
...
impl BTreeIndex {
    ...
    pub(crate) fn rebalance_leaf(&mut self, leaf: LeafNode, path: Vec<(PageId, usize)>) -> Result<(), StorageError> {
        if path.is_empty() {
    ...
    pub(super) fn try_borrow_leaf(&mut self, leaf: &LeafNode, parent: &mut InternalNode, idx: usize) -> Result<bool, StorageError> {
        // Try left sibling
        if idx > 0 {
    ...
    pub(super) fn merge_leaf(&mut self, leaf: LeafNode, parent: &mut InternalNode, idx: usize) -> Result<(), StorageError> {
        ...
        if idx > 0 {
            ...
            self.deallocate_page(leaf.page_id)?;
        } else {
            ...
            // Deallocate merged node
            self.deallocate_page(right_sibling.page_id)?;
        }
    ...
    pub(super) fn try_borrow_internal(&mut self, node: &InternalNode, parent: &mut InternalNode, idx: usize) -> Result<bool, StorageError> {
        // Try left sibling
        if idx > 0 {
    ...
    pub(super) fn merge_internal(&mut self, node: InternalNode, parent: &mut InternalNode, idx: usize) -> Result<(), StorageError> {
        // Prefer merging with left sibling
        if idx > 0 {
            let mut left_sibling = self.read_internal_node(parent.children[idx - 1])?;
            ...
            // Deallocate merged node
            self.deallocate_page(node.page_id)?;
        } else {
            // Merge with right sibling
    ...
    pub(crate) fn maybe_collapse_root(&mut self) -> Result<(), StorageError> {
        if self.height() > 1 {
            ...
            if root.children.len() == 1 {
                ...
                self.deallocate_page(old_root_id)?;
                self.set_root_and_height(new_root_id, new_height)?;
            }
        }
