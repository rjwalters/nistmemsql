...
impl BTreeIndex {
    ...
    pub(crate) fn save_metadata(&self) -> Result<(), StorageError> {
    ...
    // Helper methods for operations modules

    /// Allocate a new page
    pub(crate) fn allocate_page(&self) -> Result<PageId, StorageError> {
        self.page_manager.allocate_page()
    }

    /// Deallocate a page
    pub(crate) fn deallocate_page(&self, page_id: PageId) -> Result<(), StorageError> {
        self.page_manager.deallocate_page(page_id)
    }

    /// Set root page ID and height, then save metadata
    pub(crate) fn set_root_and_height(&mut self, root_page_id: PageId, height: usize) -> Result<(), StorageError> {
        self.root_page_id = root_page_id;
        self.height = height;
        self.save_metadata()
    }

    /// Create a new index with a specific root (used by bulk_load)
