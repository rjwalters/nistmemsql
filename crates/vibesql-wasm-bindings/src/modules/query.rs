...
impl Database {
    ...
    pub fn query(&self, sql: &str) -> Result<JsValue, JsValue> {
        ...
        let select_executor = vibesql_executor::SelectExecutor::new(&self.db);
            ...
            .map(|row| {
                ...
                let json_values: Vec<serde_json::Value> = row
                    ...
                    .map(|v| match v {
                        ...
                        vibesql_types::SqlValue::Numeric(s)
                        | vibesql_types::SqlValue::Date(s)
                        | vibesql_types::SqlValue::Time(s)
                        | vibesql_types::SqlValue::Timestamp(s)
                        | vibesql_types::SqlValue::Interval(s) => serde_json::Value::String(s.clone()),
