...
fn test_insert_not_null_constraint_with_column_list() {
    ...
    let stmt = build_insert_columns(
        "test",
        vec!["id", "age"],
        vec![types::SqlValue::Integer(1), types::SqlValue::Integer(25)],
    );
...
fn test_insert_primary_key_duplicate_single_column() {
    ...
    let stmt1 = build_insert_values(
        "users",
        vec![types::SqlValue::Integer(1), types::SqlValue::Varchar("Alice".to_string())],
    );
    ...
    let stmt2 = build_insert_values(
        "users",
        vec![types::SqlValue::Integer(1), types::SqlValue::Varchar("Bob".to_string())],
    );
...
fn test_insert_primary_key_unique_values() {
    ...
    for i in 1..=3 {
        let stmt = build_insert_values(
            "users",
            vec![types::SqlValue::Integer(i), types::SqlValue::Varchar(format!("User{}", i))],
        );
...
fn test_insert_unique_constraint_duplicate() {
    ...
    let stmt1 = build_insert_values(
        ...
        vec![
            types::SqlValue::Integer(1),
            types::SqlValue::Varchar("alice@example.com".to_string()),
        ],
    );
    InsertExecutor::execute(&mut db, &stmt1).unwrap();
...
fn test_insert_unique_constraint_allows_null() {
    ...
    for i in 1..=3 {
        let stmt =
            build_insert_values("users", vec![types::SqlValue::Integer(i), types::SqlValue::Null]);
        InsertExecutor::execute(&mut db, &stmt).unwrap();
    }
...
fn test_insert_multiple_unique_constraints() {
    ...
    let stmt3 = build_insert_values(
        ...
        vec![
            types::SqlValue::Integer(2),
            types::SqlValue::Varchar("bob@example.com".to_string()),
            types::SqlValue::Varchar("alice".to_string()),
        ],
    );
    let result = InsertExecutor::execute(&mut db, &stmt3);
...
fn test_insert_check_constraint_passes() {
    ...
    let stmt = build_insert_values(
        "products",
        vec![types::SqlValue::Integer(1), types::SqlValue::Integer(100)],
    );
...
fn test_insert_check_constraint_violation() {
    ...
    let stmt = build_insert_values(
        "products",
        vec![types::SqlValue::Integer(1), types::SqlValue::Integer(-10)],
    );
...
fn test_insert_check_constraint_with_null() {
    ...
    let stmt =
        build_insert_values("products", vec![types::SqlValue::Integer(1), types::SqlValue::Null]);
