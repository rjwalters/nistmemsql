...
fn create_scalar_subquery(table_name: &str, column_name: &str) -> Box<ast::SelectStmt> {
    Box::new(ast::SelectStmt {
        ...
        into_table: None,
        from: Some(ast::FromClause::Table { name: table_name.to_string(), alias: None }),
        where_clause: None,
        group_by: None,
        having: None,
        order_by: None,
...
fn test_update_with_scalar_subquery_avg_aggregate() {
    ...
    insert_employee(&mut db, 1, SqlValue::Integer(10000));
    insert_data_row(&mut db, "salaries", SqlValue::Integer(60000));
    ...
    let count = UpdateExecutor::execute(&stmt, &mut db).unwrap();
    assert_eq!(count, 1);
    verify_single_row_update(&db, "employees", 1, SqlValue::Integer(60000));
}
