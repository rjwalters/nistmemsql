    ...
    fn execute_add_constraint(
        ...
        match &stmt.constraint.kind {
            ...
            TableConstraintKind::ForeignKey {
                ...
                use catalog::ForeignKeyConstraint;

                // Convert AST ReferentialAction to catalog ReferentialAction
                let convert_action = |action: &Option<ast::ReferentialAction>| {
                    match action {
                        Some(ast::ReferentialAction::Cascade) => {
                            catalog::ReferentialAction::Cascade
                        }
                        Some(ast::ReferentialAction::SetNull) => {
                            catalog::ReferentialAction::SetNull
                        }
                        Some(ast::ReferentialAction::SetDefault) => {
                            catalog::ReferentialAction::SetDefault
                        }
                        Some(ast::ReferentialAction::NoAction) | None => {
                            catalog::ReferentialAction::NoAction
                        }
                    }
                };
