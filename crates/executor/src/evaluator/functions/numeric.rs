...
pub(super) fn format(args: &[types::SqlValue]) -> Result<types::SqlValue, ExecutorError> {
    if args.len() != 2 {
        return Err(ExecutorError::UnsupportedFeature(
    ...
    match (&args[0], &args[1]) {
        ...
        (number_val, types::SqlValue::Integer(decimals)) => {
            let decimals = (*decimals).max(0) as usize;

            // Convert number to f64
            let num = match number_val {
                types::SqlValue::Integer(n) => *n as f64,
                types::SqlValue::Bigint(n) => *n as f64,
                types::SqlValue::Smallint(n) => *n as f64,
                types::SqlValue::Float(f) => *f as f64,
                types::SqlValue::Double(f) => *f,
                types::SqlValue::Real(f) => *f as f64,
