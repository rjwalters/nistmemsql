...
impl<'a> CombinedExpressionEvaluator<'a> {
    ...
    pub(crate) fn eval(
        ...
        match expr {
            ...
            // Unary operations (delegate to shared eval_unary_op_static)
            ast::Expression::UnaryOp { op, expr } => {
                let val = self.eval(expr, row)?;
                Self::eval_unary_op_static(op, &val)
            }
    ...
    /// Static version of eval_unary_op for shared logic (used by CombinedExpressionEvaluator)
    fn eval_unary_op_static(
