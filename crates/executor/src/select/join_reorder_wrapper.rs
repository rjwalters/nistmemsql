...
fn count_tables(from: &ast::FromClause) -> usize {
    match from {
        ast::FromClause::Table { .. } => 1,
        ast::FromClause::Subquery { .. } => 1,
        ast::FromClause::Join { left, right, .. } => count_tables(left) + count_tables(right),
    }
}

/// Extract table names from a FROM clause in left-to-right order
fn extract_table_names(from: &ast::FromClause, tables: &mut Vec<String>) {
    match from {
