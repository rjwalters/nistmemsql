...
pub(super) fn evaluate(
    ...
    let (name, args, character_unit) = match expr {
        ast::Expression::Function { name, args, character_unit } => (name, args, character_unit),
    ...
    let literal_args: Vec<ast::Expression> =
        temp_row.values.iter().map(|val| ast::Expression::Literal(val.clone())).collect();
