...
fn test_count_star_multiple_select_items_no_fast_path() {
    ...
    let result = executor.execute(&stmt).unwrap();
    assert_eq!(result.len(), 1);
    assert_eq!(result[0].values[0], types::SqlValue::Integer(5)); // COUNT(*) returns Integer
    assert_eq!(result[0].values[1], types::SqlValue::Numeric(100.0)); // SUM returns Numeric
}
