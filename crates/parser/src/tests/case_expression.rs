...
fn test_parse_searched_case_simple() {
    ...
    if let ast::Statement::Select(select) = stmt {
        ...
        if let ast::SelectItem::Expression { expr, .. } = &select.select_list[0] {
            if let ast::Expression::Case { operand, when_clauses, else_result } = expr {
...
fn test_parse_searched_case_no_else() {
    ...
    if let ast::Statement::Select(select) = stmt {
        if let ast::SelectItem::Expression { expr, .. } = &select.select_list[0] {
            if let ast::Expression::Case { else_result, .. } = expr {
...
fn test_parse_case_with_alias() {
    ...
    let stmt = result.unwrap();
    if let ast::Statement::Select(select) = stmt {
        if let ast::SelectItem::Expression { alias, .. } = &select.select_list[0] {
...
fn test_parse_case_in_where() {
    ...
    if let ast::Statement::Select(select) = stmt {
        ...
        if let ast::Expression::Case { .. } = &select.where_clause.unwrap() {
            // Success
        } else {
...
fn test_parse_case_web_demo_query() {
    ...
    if let ast::Statement::Select(select) = stmt {
        ...
        if let ast::SelectItem::Expression { expr, alias } = &select.select_list[2] {
            if let ast::Expression::Case { operand, when_clauses, else_result } = expr {
