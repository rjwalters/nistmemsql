            ...
            match &select.select_list[0] {
                ast::SelectItem::Expression { expr, .. } => match expr {
                    ast::Expression::WindowFunction { function, over } => {
                        match function {
                            ast::WindowFunctionSpec::Ranking { name, .. } => {
        ...
        ast::Statement::Select(select) => match &select.select_list[0] {
            ast::SelectItem::Expression { expr, .. } => match expr {
                ast::Expression::WindowFunction { over, .. } => {
                    ...
                    assert_eq!(order[0].direction, ast::OrderDirection::Desc);
                }
                _ => panic!("Expected WindowFunction"),
            },
            _ => panic!("Expected Expression"),
        ...
        ast::Statement::Select(select) => match &select.select_list[0] {
            ast::SelectItem::Expression { expr, .. } => match expr {
                ast::Expression::WindowFunction { over, .. } => {
                    ...
                    assert_eq!(order[0].direction, ast::OrderDirection::Asc);
                    assert_eq!(order[1].direction, ast::OrderDirection::Desc);
                }
                _ => panic!("Expected WindowFunction"),
            },
            _ => panic!("Expected Expression"),
        ...
        ast::Statement::Select(select) => match &select.select_list[0] {
            ast::SelectItem::Expression { expr, .. } => match expr {
                ast::Expression::WindowFunction { over, .. } => {
                    ...
                    assert_eq!(frame.unit, ast::FrameUnit::Rows);
                    assert_eq!(frame.start, ast::FrameBound::UnboundedPreceding);
                }
                _ => panic!("Expected WindowFunction"),
            },
            _ => panic!("Expected Expression"),
        ...
        ast::Statement::Select(select) => match &select.select_list[0] {
            ast::SelectItem::Expression { expr, .. } => match expr {
                ast::Expression::WindowFunction { over, .. } => {
                    ...
                    match &frame.start {
                        ast::FrameBound::Preceding(_) => {}
                        _ => panic!("Expected Preceding frame start"),
                    }

                    assert!(frame.end.is_some());
                    assert_eq!(frame.end.as_ref().unwrap(), &ast::FrameBound::CurrentRow);
                }
                _ => panic!("Expected WindowFunction"),
            },
            _ => panic!("Expected Expression"),
        ...
        ast::Statement::Select(select) => match &select.select_list[0] {
            ast::SelectItem::Expression { expr, .. } => match expr {
                ast::Expression::WindowFunction { over, .. } => {
                    assert!(over.frame.is_some());
                    let frame = over.frame.as_ref().unwrap();
                    assert_eq!(frame.unit, ast::FrameUnit::Range);
                }
                _ => panic!("Expected WindowFunction"),
            },
            _ => panic!("Expected Expression"),
...
fn test_aggregate_as_window_function() {
    ...
    match stmt {
        ast::Statement::Select(select) => match &select.select_list[0] {
            ast::SelectItem::Expression { expr, .. } => match expr {
                ast::Expression::WindowFunction { function, .. } => match function {
        ...
        ast::Statement::Select(select) => match &select.select_list[0] {
            ast::SelectItem::Expression { expr, .. } => match expr {
                ast::Expression::WindowFunction { over, .. } => {
                    ...
                    match &frame.end {
                        Some(ast::FrameBound::Following(_)) => {}
