...
fn test_substring_comma_syntax() {
    ...
    if let ast::Statement::Select(select) = stmt {
        ...
        if let ast::SelectItem::Expression { expr, .. } = &select.select_list[0] {
            if let ast::Expression::Function { name, args, character_unit: _ } = expr {
...
fn test_substring_syntaxes_equivalent() {
    ...
    if let (
        ast::Expression::Function { name: name1, args: args1, character_unit: _ },
        ast::Expression::Function { name: name2, args: args2, character_unit: _ },
