...
fn main() {
    ...
    for insert_sql in inserts {
        ...
        match stmt {
            ...
            _ => panic!("Not an INSERT statement"),
        }
    }

    // Test ORDER BY on a simple column first
    let simple_query = "SELECT a FROM t1 ORDER BY a";
    println!("Running simple query: {}", simple_query);

    let stmt = Parser::parse_sql(simple_query).unwrap();
    match stmt {
        ast::Statement::Select(select_stmt) => {
            ...
            match result {
                Ok(rows) => {
                    println!("Simple query succeeded, {} rows:", rows.len());
                    for row in rows {
                        if let SqlValue::Integer(val) = row.values[0] {
                            print!("{} ", val);
                        }
                    }
                    println!();
    ...
    println!("Running complex query: {}", query_sql);
