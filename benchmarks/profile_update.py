...
def profile_vibesql_update(num_rows=1000):
    """Profile vibesql UPDATE operation."""
    db = vibesql.connect()
    cursor = db.cursor()
...
def test_single_update_overhead():
    ...
    print(f"vibesql single UPDATE: {sum(times)/len(times):.3f} ms (avg of 10)")
...
if __name__ == "__main__":
    ...
    nist_time = profile_vibesql_update(1000)
    sqlite_time = profile_sqlite_update(1000)

    print("\n" + "=" * 70)
    print("SUMMARY")
    print("=" * 70)
    print(f"vibesql total: {nist_time*1000:.2f} ms")
